...
public class AppSettingsService {
    ...
    private <T> Function<T, T> configure(final AppSettingsConfigurer<T> configurer) {
        return action -> configurer.isApplicable(action) ?
    ...
    private <T> Stream<T> getStaticSettingsStream(final AppSettingsProvider<T>[] providers) {
        return stream(providers)
                .map(AppSettingsProvider::getSettings)
     ...
     */
    private <T> Stream<T> getSettingsStream(
            final AppSettingsProvider<T>[] providers,
