...
public class DataSetService extends BaseDataSetService {
    ...
    public String updateRawDataSet(
        ...
        if (currentDataSetMetadata == null) {
            ...
            try {
                ...
                if (size != null && size > 0) {
                    quotaService
                            .checkIfAddingSizeExceedsAvailableStorage(Math.abs(size - currentDataSetMetadata.getDataSetSize()));
                }
        ...
        if (dataSetMetadata != null) {
            ...
            if (matchingDatasetLocation == null) {
                ...
                if (matchingDatasetLocation.isSchemaOriented()) {
                    ...
                    parametersAsSchema
                            .setProperties(dataSetMetadata.getLocation().getParametersAsSchema(getLocale()).getProperties());
    ...
    private void checkDataSetName(String dataSetName) {
        if (dataSetName == null || dataSetName.contains("'")) {
            throw new TDPException(INVALID_DATASET_NAME, ExceptionContext.withBuilder().put("name", dataSetName).build());
        }
    }
