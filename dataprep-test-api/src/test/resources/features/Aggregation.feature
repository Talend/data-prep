...
Feature: Make a aggregation
  ...
  Scenario: Make an aggregation on a preparation (Average)
    ...
    When I apply an aggregation on the preparation "scores_preparation" with parameters :
      | operator | AVERAGE |
      | columnId | 0001    |
      | groupBy  | 0002    |
  ...
  Scenario: Make an aggregation on a preparation (Min)
    ...
    Then The aggregate result with the operator "MIN" is :
      | FRONT | 12.0 |
      | BACK  | 12.0 |
  ...
  Scenario: Make an aggregation on a preparation (Max)
    ...
    Then The aggregate result with the operator "MAX" is :
      | BACK  | 20.0 |
      | FRONT | 19.0 |
  ...
  Scenario: Make an aggregation on a preparation (Sum)
    ...
    Then The aggregate result with the operator "SUM" is :
      | BACK  | 69.0 |
      | FRONT | 63.0 |
  ...
  Scenario: Make an aggregation on a dataSet (Min)
    When I apply an aggregation on the dataSet "scores_dataset" with parameters :
      | operator | MIN     |
      | columnId | 0001    |
      | groupBy  | 0002    |
